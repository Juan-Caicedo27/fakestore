...
function render() {
  ...
  const title = createElement('h2', {}, 'Adivina el producto');
...
async function fetchRandomProducto() {
  app.innerHTML = '<p>Cargando producto...</p>';
  try {
    const res = await fetch('https://fakestoreapi.com/products');
    const data = await res.json();
    const random = data[Math.floor(Math.random() * data.length)];
    pokemonName = random.title.toUpperCase(); // ahora el nombre del producto
    pokemonId = random.id;
